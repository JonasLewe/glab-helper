...
func TestWritePreviewIsReadOnlyAndShowsRelationships(t *testing.T) {
	plan := Plan{
		Actions: []Action{
			...
			{Operation: Create, Desired: DesiredItem{Target: BoardList, Title: "status::In Progress", BoardID: 3, BoardName: "Development", LabelID: 4}},
	...
	for _, want := range []string{
		...
		`CREATE label "team-a"`,
		`CREATE board list "status::In Progress" on "Development"`,
		...
		"description: changed",
		`state: "opened" -> "closed"`,
		"Summary: 3 to create, 1 to update, 1 unchanged, 2 ignored source items.",
